...
<body class="bg-slate-300 h-screen flex justify-center items-center">
  ...
  <form method="post" action="{{ route('categorie.store') }}" class="bg-white p-6 rounded-xl shadow-md space-y-4">
    @csrf
    <h1 class="text-2xl font-bold text-center mb-4">Ajouter une catégorie</h1>
    ...
    <label class="block">
      ...
      <select id="CategorieFils" name="Categorie_Fils" class="border p-2 rounded-md w-full" required>
        <option value="">-- Choisir d'abord une catégorie père --</option>
      </select>
    </label>
